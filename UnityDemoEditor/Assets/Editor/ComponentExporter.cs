...
namespace f.DemoExporter {
	public class ComponentExporter {
		...
		private string Serialize(MeshRenderer mr, AssetManager assetManager) {
			string transformTemplate = ", sharedMaterial: {0}";
			return string.Format(transformTemplate, Serialize(mr.sharedMaterial, assetManager));
		}
		...
		private string Serialize(MonoBehaviour mb, IObjectContainer objs, AssetManager assetManager) {
			...
			var script = JsCodeExtractor.GetScript(mb);
			if (script != null) {
		...
		private string Serialize(Camera camera, AssetManager am, IObjectContainer objc) {
			string cameraTemplate = ", projection: \"{1}\", fov: {2}, renderOrder: {3}, clear: \"{4}\", clearColor: {5}, near: {6}, far: {7}, viewport: {8} {9}";
			string extra = "";
			if (camera.GetComponent<RenderTexture> () != null) {
				extra = string.Format(",\nrenderFullScreenTextureMat: {0},\nrenderCameraToTextureRenderer:{1}", 
				                      am.RegisterAsset(camera.GetComponent<RenderTexture>().mat),
				                      objc.GetObjectRefId(camera.GetComponent<RenderTexture>().renderer));
			}
			if (camera.GetComponent<RenderCameraToTexture>() != null) {
				extra = extra + string.Format(",\nrenderToTexture: true");
			}
			return string.Format(cameraTemplate, "camera", camera.isOrthoGraphic ? "ortho" : "perspective", 
			                     camera.fieldOfView, camera.depth, camera.clearFlags.ToString(), Serialize(camera.backgroundColor),
			                     camera.nearClipPlane, camera.farClipPlane, Serialize (camera.rect), extra);
		}
