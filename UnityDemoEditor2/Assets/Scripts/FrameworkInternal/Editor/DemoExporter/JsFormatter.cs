...
public class JsFormatter {
	...
	private static string [] SplitNoStrings(string str, char delimiter) {
		...
		string curr = "";
		//bool inString = false;
		char ? inStringDelimiter = null;
		while (N < str.Length) {
			char c = str[N];
			if (!inStringDelimiter.HasValue && IsStringDelimiter(c)) {
				//inString = !inString;
				inStringDelimiter = c;
			} else 
			if (inStringDelimiter.HasValue && c == inStringDelimiter) {
				inStringDelimiter = null;
			}

			if (c == delimiter && !inStringDelimiter.HasValue) {
				strings.Add(curr);
				curr = "";
			} else {
				curr += c;
